...
- 3段階の考え方は既存の芽のイラストと各一文で説明。施術工程の写真は対応関係が異なるため、この箇所から削除しました。
- 施術の流れは写真を大きくした2列レイアウト。予約は番号を軸にした左揃えの3ステップ。
...
- 390 px で主な改修4セクションの初期表示文字数: 3,254 → 2,184（約33%削減）。HTMLのテキストで比較し、開閉詳細は閉じた状態。
- ページ内の画像要素: 33 → 37。追加分は説明イラスト3点と肌診断工程の既存カウンセリング写真1点。ほかに同じ線幅・配色のコードネイティブ図解を追加。
...
- 肌診断工程のクリップボードアイコンを既存のカウンセリング写真へ変更。
- 「呼び起こす・育てる・守り抜く」の施術写真3点と小見出しを外し、既存の芽のイラストと短い一文に整理。アイコンは本文の横に置き、上部の余白を増やしていません。
- 使用素材・ケア成分の開閉、実際の施術工程の写真・説明は維持しています。
- 予約案内の大きなスマートフォン線画3点を削除。落ち着いた番号、明朝体の見出し、細い区切り線で構成し、狭い画面でも文節を分断しない縦並びへ変更。
- 予約案内の本文・価格・LINEリンクは変更せず、改行と強調のみ調整。6868f0fの本文との一致を検証。
